---
title: "Final Paper - Analysis on Global Happiness"
...
map.world[map.world == "USA"] <- "United States"
map.world[map.world == "UK"] <- "United Kingdom"
map.world[map.world == "Republic of Congo"] <- "Congo (Brazzaville)"
...
Using a multiple regression model we were able to determine the best fit model for the `Ladder score` based on the other recorded variables. 

```{r echo=F}
model1 <- lm(Score ~ Freedom + GDP + Social_Support + Health + Generosity + Corruption, data = represent_data)
avPlots(model1, main=paste(""))
```
